...
if (isset($_POST['a_remover']) || isset($_POST['a_ativar']) || isset($_POST['a_desativar']) || isset($_POST['a_copiar'])) {
	if (isset($_POST['f_opts'])) {
		...
		foreach ($_POST['f_opts'] as $op) {
			$idop = $website->validate_hash('sdk_produtos', $op);
			if ($idop) {
				$opts_arr[] = $idop;
			} else {
		...
		if (count($opts_arr) > 0) {
			if (isset($_POST['a_remover'])) {
				foreach ($opts_arr as $op) {
					$website->remove_db_id('sdk_produtos', $op);
				}
				...
				foreach ($opts_arr as $op) {
					$sql = "UPDATE sdk_produtos SET dt_ativado=1 WHERE ".$website->db_id."=".$op;
					$website->sql_db($sql);
				}
				...
				foreach ($opts_arr as $op) {
					$sql = "UPDATE sdk_produtos SET dt_ativado=0 WHERE ".$website->db_id."=".$op;
					$website->sql_db($sql);
				}
				...
				foreach ($opts_arr as $op) {
					$query = "SELECT * FROM sdk_produtos WHERE ".$website->db_id."=".$op;
...
if (isset($_REQUEST['l_status']) && isset($_REQUEST['f'])) {
	...
	$query = "SELECT * FROM sdk_produtos WHERE ".$website->db_id."=".$opts_id;
	$result = $website->sql_db($query);
	$line = $website->fetch_array_db($result);
	
	$new_v = ($line['dt_ativado'] == 1) ? 0 : 1;
	
	$sql = "UPDATE sdk_produtos SET dt_ativado=".$new_v." WHERE dt_id=".$opts_id;
	$website->sql_db($sql);	
...
$query = "SELECT * FROM sdk_produtos ORDER BY dt_id DESC";
$result = $website->sql_db($query);
    	...
    	<div class="col-xs-12 rt_cntbdy">
          ...
          <form action="produtos.php" method="post">
        	...
        	<div class="pull-left rt_a">
                <a class="btn btn-primary" href="produtos_inserir.php">+ <?php echo $website->get_string(18); ?></a>
            </div>
              ...
              <table class="table table-striped table-hover table-responsive">
                <thead class="rt_tbrw">
                  <tr>
                    ...
                    <th class="text-center"><a class="rg_g" href="produtos.php?<?php echo 'q='.$website->query.'&'; ?>o=7&order=<?php echo ($website->order == 7 && $website->order_direction == 'asc') ? 'desc' : 'asc'; ?>"><?php echo $website->get_string(32); ?><?php if ($website->order == 7) { if ($website->order_direction == 'asc') { ?>&nbsp;<i class="fa fa-sort-amount-asc"></i><?php } else { ?>&nbsp;<i class="fa fa-sort-amount-desc"></i><?php } } ?></a></th>
                  </tr>
                </thead>
                <tbody>
            ...
            <div class="text-center">
                <ul class="pagination">
                  <li<?php if ($website->page == 1) echo ' class="disabled"'; ?>><a href="<?php if ($website->page == 1) { echo '#'; } else { ?>produtos.php?<?php echo 'p=1&'.'q='.$website->query.'&'.'o='.$website->order.'&'.'order='.$website->order_direction.'&'; } ?>">&laquo;&nbsp;&laquo;</a></li>
                  <li<?php if ($website->page == 1) echo ' class="disabled"'; ?>><a href="<?php if ($website->page == 1) { echo '#'; } else { ?>produtos.php?<?php echo 'p='.($website->page - 1).'&'.'q='.$website->query.'&'.'o='.$website->order.'&'.'order='.$website->order_direction.'&'; } ?>">&laquo;</a></li>
                  <?php for ($x = $website->page_arr['pagina_bloco_inicio']; $x <= $website->page_arr['pagina_bloco_final']; $x++) { ?>
                  <li<?php if ($website->page == $x) echo ' class="active"'; ?>><a href="<?php if ($website->page == $x) { echo '#'; } else { ?>produtos.php?<?php echo 'p='.$x.'&'.'q='.$website->query.'&'.'o='.$website->order.'&'.'order='.$website->order_direction.'&'; } ?>"><?php echo $x; ?></a></li>
                  <?php } ?>
                  <li<?php if ($website->page == $website->page_arr['pagina_total']) echo ' class="disabled"'; ?>><a href="<?php if ($website->page == $website->page_arr['pagina_total']) { echo '#'; } else { ?>produtos.php?<?php echo 'p='.($website->page + 1).'&'.'q='.$website->query.'&'.'o='.$website->order.'&'.'order='.$website->order_direction.'&'; } ?>">&raquo;</a></li>
                  <li<?php if ($website->page == $website->page_arr['pagina_total']) echo ' class="disabled"'; ?>><a href="<?php if ($website->page == $website->page_arr['pagina_total']) { echo '#'; } else { ?>produtos.php?<?php echo 'p='.$website->page_arr['pagina_total'].'&'.'q='.$website->query.'&'.'o='.$website->order.'&'.'order='.$website->order_direction.'&'; } ?>">&raquo;&nbsp;&raquo;</a></li>
                </ul>
            </div>
          </form>
        </div>
