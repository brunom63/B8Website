...
if (isset($_POST['a_remover']) || isset($_POST['a_ativar']) || isset($_POST['a_desativar']) || isset($_POST['a_copiar']) || isset($_POST['a_enviar'])) {
	if (isset($_POST['f_opts'])) {
		$opts_arr = array();
		foreach ($_POST['f_opts'] as $op) {
			$idop = $website->validate_hash('sdk_newsletters', $op);
			if ($idop) {
				$opts_arr[] = $idop;
			} else {
		...
		if (count($opts_arr) > 0) {
			if (isset($_POST['a_remover'])) {
				foreach ($opts_arr as $op) {
					$website->remove_db_id('sdk_newsletters', $op);
				}
				...
				foreach ($opts_arr as $op) {
					$sql = "UPDATE sdk_newsletters SET dt_ativado=1 WHERE ".$website->db_id."=".$op;
					$website->sql_db($sql);
				}
				...
				foreach ($opts_arr as $op) {
					$sql = "UPDATE sdk_newsletters SET dt_ativado=0 WHERE ".$website->db_id."=".$op;
					$website->sql_db($sql);
				}
				...
				foreach ($opts_arr as $op) {
					$query = "SELECT * FROM sdk_newsletters WHERE ".$website->db_id."=".$op;
					...
					$sql = "INSERT INTO sdk_newsletters VALUES (NULL, '".$website->db_hashfield('sdk_newsletters')."', '".$line['dt_pai']."', '".$line['dt_titulo'].' - NOVO'."', '".$line['dt_alias']."', '".$line['dt_autor']."', ".$dates_arr['dt_data_inicio'].", ".$dates_arr['dt_data_fim'].", '".$line['dt_ativado']."', '".$line['dt_destaque']."', ".$website->p_id.", ".$website->data_hj.", NULL, '".$line['dt_corpo']."', NULL, NULL, NULL, '".$line['dt_meta_descricao']."', '".$line['dt_meta_tags']."', 0, 0, NULL)";
					$website->sql_db($sql);
				}
		
				$website->str_error = $website->get_string(96);
				$website->str_error_type = 1;		
			} else if (isset($_POST['a_enviar'])) {
                $url_arr = explode("/", $_SERVER['REQUEST_URI']);
                $url = 'http://'.$_SERVER['HTTP_HOST'].'/'.$url_arr[1].'/newsletters_send.php?c='.urlencode($_COOKIE[$website->session_id]).'&h='.$_SERVER['HTTP_HOST'].'&o='.implode(",", $opts_arr);
                $data = file_get_contents($url);

				$website->str_error = $website->get_string(109);
				$website->str_error_type = 1;		
			} else {
				$website->page_redirect('atencao.php');
			}
...
if (isset($_REQUEST['l_status']) && isset($_REQUEST['f'])) {
	$opts_id = $website->validate_hash('sdk_newsletters', $_REQUEST['f']);
	if (!$opts_id) $website->page_redirect('atencao.php');
	
	$query = "SELECT * FROM sdk_newsletters WHERE ".$website->db_id."=".$opts_id;
	$result = $website->sql_db($query);
	$line = $website->fetch_array_db($result);
	
	$new_v = ($line['dt_ativado'] == 1) ? 0 : 1;
	
	$sql = "UPDATE sdk_newsletters SET dt_ativado=".$new_v." WHERE dt_id=".$opts_id;
	$website->sql_db($sql);	
...
if (isset($_REQUEST['l_destaque']) && isset($_REQUEST['f'])) {
	...
	$sql = "UPDATE sdk_newsletters SET dt_destaque=".$new_v." WHERE dt_id=".$opts_id;
	$website->sql_db($sql);
...
$query = "SELECT * FROM sdk_newsletters ORDER BY dt_id DESC";
$result = $website->sql_db($query);
...
$website->items = $website->order_results($website->items, array(1 => 'dt_titulo', 2 => 'dt_ativado', 3 => 'dt_destaque',
										4 => 'dt_pai_titulo', 5 => 'dt_criado_data', 6 => 'dt_hits', 7 => 'dt_id', 8 => 'dt_enviados'));
    ...
    <div class="col-xs-10">
	  <div class="row">
    	<div class="col-xs-12 rt_cnttit">
          <h1><i class="fa fa-envelope fa-2x"></i><span class="rt_cnttxt"><?php echo $website->get_string(6); ?></span><?php $website->call_back('main.php'); ?></h1>
        </div>
      </div>
      <div class="row">
    	<div class="col-xs-12 rt_cntbdy">
          ...
          <form action="newsletters.php" method="post">
        	...
        	<div class="pull-left rt_a">
                <a class="btn btn-primary" href="newsletters_inserir.php">+ <?php echo $website->get_string(18); ?></a>
            </div>
        	...
        	<div class="pull-right rt_a">
              <button type="submit" class="btn btn-primary" name="a_enviar"><?php echo $website->get_string(20); ?></button>
              ...
              <table class="table table-striped table-hover table-responsive">
                <thead class="rt_tbrw">
                  <tr>
                    ...
                    <th><a class="rg_g" href="newsletters.php?<?php echo 'q='.$website->query.'&'; ?>o=1&order=<?php echo ($website->order == 1 && $website->order_direction == 'asc') ? 'desc' : 'asc'; ?>"><?php echo $website->get_string(26); ?><?php if ($website->order == 1) { if ($website->order_direction == 'asc') { ?>&nbsp;<i class="fa fa-sort-amount-asc"></i><?php } else { ?>&nbsp;<i class="fa fa-sort-amount-desc"></i><?php } } ?></a></th>
                    <th class="text-center"><a class="rg_g" href="newsletters.php?<?php echo 'q='.$website->query.'&'; ?>o=2&order=<?php echo ($website->order == 2 && $website->order_direction == 'asc') ? 'desc' : 'asc'; ?>"><?php echo $website->get_string(27); ?><?php if ($website->order == 2) { if ($website->order_direction == 'asc') { ?>&nbsp;<i class="fa fa-sort-amount-asc"></i><?php } else { ?>&nbsp;<i class="fa fa-sort-amount-desc"></i><?php } } ?></a></th>
                    ...
                    <th class="text-center"><a class="rg_g" href="newsletters.php?<?php echo 'q='.$website->query.'&'; ?>o=4&order=<?php echo ($website->order == 4 && $website->order_direction == 'asc') ? 'desc' : 'asc'; ?>"><?php echo $website->get_string(33); ?><?php if ($website->order == 4) { if ($website->order_direction == 'asc') { ?>&nbsp;<i class="fa fa-sort-amount-asc"></i><?php } else { ?>&nbsp;<i class="fa fa-sort-amount-desc"></i><?php } } ?></a></th>
                    ...
                    <th class="text-center"><a class="rg_g" href="newsletters.php?<?php echo 'q='.$website->query.'&'; ?>o=6&order=<?php echo ($website->order == 8 && $website->order_direction == 'asc') ? 'desc' : 'asc'; ?>"><?php echo $website->get_string(34); ?><?php if ($website->order == 8) { if ($website->order_direction == 'asc') { ?>&nbsp;<i class="fa fa-sort-amount-asc"></i><?php } else { ?>&nbsp;<i class="fa fa-sort-amount-desc"></i><?php } } ?></a></th>
                ...
                <?php for ($x = $website->page_arr['pagina_inicio']; $x <= $website->page_arr['pagina_final']; $x++) { $item = $website->items[$x]; ?>
                  <tr>
                    ...
                    <td><a href="newsletters_editar.php?i=<?php echo $item['dt_hash']; ?>"><?php echo $item['dt_titulo']; ?></a></td>
                    <td class="text-center"><a href="newsletters.php?<?php echo 'p='.$website->page.'&'.'q='.$website->query.'&'.'o='.$website->order.'&'.'order='.$website->order_direction.'&'; ?>l_status=&f=<?php echo $item['dt_hash']; ?>"><?php echo ($item['dt_ativado'] == 1) ? '<i class="fa fa-check-circle text-success"></i>' : '<i class="fa fa-circle text-danger"></i>'; ?></a></td>
                    <td class="text-center"><a href="newsletters.php?<?php echo 'p='.$website->page.'&'.'q='.$website->query.'&'.'o='.$website->order.'&'.'order='.$website->order_direction.'&'; ?>l_destaque=&f=<?php echo $item['dt_hash']; ?>"><?php echo ($item['dt_destaque'] == 1) ? '<i class="fa fa-check-circle text-success"></i>' : '<i class="fa fa-circle text-danger"></i>'; ?></a></td>
                    ...
                    <td class="text-center"><a href="newsletters_relatorio.php?i=<?php echo $item['dt_hash']; ?>" title="<?php echo $website->get_string(108); ?>"><?php echo $item['dt_enviados']; ?></a></td>
            ...
            <div class="text-center">
                <ul class="pagination">
                  <li<?php if ($website->page == 1) echo ' class="disabled"'; ?>><a href="<?php if ($website->page == 1) { echo '#'; } else { ?>newsletters.php?<?php echo 'p=1&'.'q='.$website->query.'&'.'o='.$website->order.'&'.'order='.$website->order_direction.'&'; } ?>">&laquo;&nbsp;&laquo;</a></li>
                  <li<?php if ($website->page == 1) echo ' class="disabled"'; ?>><a href="<?php if ($website->page == 1) { echo '#'; } else { ?>newsletters.php?<?php echo 'p='.($website->page - 1).'&'.'q='.$website->query.'&'.'o='.$website->order.'&'.'order='.$website->order_direction.'&'; } ?>">&laquo;</a></li>
                  <?php for ($x = $website->page_arr['pagina_bloco_inicio']; $x <= $website->page_arr['pagina_bloco_final']; $x++) { ?>
                  <li<?php if ($website->page == $x) echo ' class="active"'; ?>><a href="<?php if ($website->page == $x) { echo '#'; } else { ?>newsletters.php?<?php echo 'p='.$x.'&'.'q='.$website->query.'&'.'o='.$website->order.'&'.'order='.$website->order_direction.'&'; } ?>"><?php echo $x; ?></a></li>
                  <?php } ?>
                  <li<?php if ($website->page == $website->page_arr['pagina_total']) echo ' class="disabled"'; ?>><a href="<?php if ($website->page == $website->page_arr['pagina_total']) { echo '#'; } else { ?>newsletters.php?<?php echo 'p='.($website->page + 1).'&'.'q='.$website->query.'&'.'o='.$website->order.'&'.'order='.$website->order_direction.'&'; } ?>">&raquo;</a></li>
                  <li<?php if ($website->page == $website->page_arr['pagina_total']) echo ' class="disabled"'; ?>><a href="<?php if ($website->page == $website->page_arr['pagina_total']) { echo '#'; } else { ?>newsletters.php?<?php echo 'p='.$website->page_arr['pagina_total'].'&'.'q='.$website->query.'&'.'o='.$website->order.'&'.'order='.$website->order_direction.'&'; } ?>">&raquo;&nbsp;&raquo;</a></li>
                </ul>
            </div>
          </form>
        </div>
